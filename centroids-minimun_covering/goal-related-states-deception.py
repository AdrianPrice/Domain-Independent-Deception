...
def parse_options():
    ...
    parser.add_option('-c', '--criterion', action='store', dest='criterion',
                      help='criterion for selecting a goal in the set of candidate goals: CLOSEST or FARTHEST', default='CLOSEST')
...
def re_write_goals():
    ...
    for g in goals:
        ...
        goals_to_distance[str(g)] = cost
        cont += 1


    # print(goals_to_distance)

    if criterion == 'CLOSEST':
        g_distance = sys.maxint
    elif criterion == 'FARTHEST':
        g_distance = 0
    
    selected_goal = None
    
    for g in goals_to_distance.keys():
        if criterion == 'CLOSEST':
            if goals_to_distance[g] < g_distance:
                selected_goal = g
                g_distance = goals_to_distance[g]
        elif criterion == 'FARTHEST':
            if goals_to_distance[g] > g_distance:
                selected_goal = g
                g_distance = goals_to_distance[g]        
    ...
    selected_goal_s = selected_goal.replace('[', '')
    selected_goal_s = selected_goal_s.replace('\']', '')
    selected_goal_s = selected_goal_s.replace('\'(', '(')
    selected_goal_list = selected_goal_s.split('\', ')
    # print(selected_goal_list)

    selected_goal_to_file = ''
    for i in range(0,len(selected_goal_list)):
        selected_goal_to_file += selected_goal_list[i] + '|' if i < len(selected_goal_list)-1 else selected_goal_list[i]
    
    selected_goal_to_file = selected_goal_to_file + ' - 1.0'

    # print(selected_goal_to_file)

    path_to_save_files = options.domain_file.replace('/domain.pddl', '')
    real_hyp = ''
    real_hype_file = open(path_to_save_files + '/real_hyp.dat', 'r')
    ...
    real_hyp_to_file = real_hyp.replace('(and ', '')
    real_hyp_to_file = real_hyp_to_file.replace('(AND ', '')
    real_hyp_to_file = real_hyp_to_file.replace(') (', ')|(')
    real_hyp_to_file = real_hyp_to_file.replace('))', ') - 1.0')
    # print(real_hyp_to_file)

    criterion_file_name = path_to_save_files + '/goals-real_goal-farthest_goal.txt'
    if criterion == 'CLOSEST':
        criterion_file_name = path_to_save_files + '/goals-real_goal-closest_goal.txt'


    new_goals_file = open(criterion_file_name, 'w')
    new_goals_file.write(selected_goal_to_file + '\n')
    new_goals_file.write(real_hyp_to_file)
    new_goals_file.close()

    return criterion_file_name
...
def main():
    ...
    criterion = options.criterion
    options.goals_file = re_write_goals()
    ...
    if criterion == 'CLOSEST':
        output_file_state_name = pb_number + '_' + domain_name + "_" + options.state + "_" + options.estimation + "_" + options.exploration + "-real_goal-closest_goal.txt"
        output_file_state_name_runtime = pb_number + '_' + domain_name + "_" + options.state + "_" + options.estimation + "_" + options.exploration + "-real_goal-closest_goal-runtime.txt"
    else: 
        output_file_state_name = pb_number + '_' + domain_name + "_" + options.state + "_" + options.estimation + "_" + options.exploration + "-real_goal-farthest_goal.txt"
        output_file_state_name_runtime = pb_number + '_' + domain_name + "_" + options.state + "_" + options.estimation + "_" + options.exploration + "-real_goal-farthest_goal-runtime.txt"
