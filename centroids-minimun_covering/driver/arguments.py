...
def parse_args():
    ...
    driver_other.add_argument("--state",
                              help="state we want to compute: centroid,minimum-covering,medoid,minimum-covering-m,r-centroid,r-minimum-covering,r-medoid,r-minimum-covering-m")

    driver_other.add_argument("--estimation",
                              help="method employed to estimate the cost of reaching its goal: heuristic,optimal-plan")

    driver_other.add_argument("--exploration",
                              help="exploratory mode: all-reachable,greedy,greedy-restart,all-reachable-fast")

    driver_other.add_argument("--search-paradigm",
                              help="search paradigm: forward,backward")

    driver_other.add_argument("--goals-file",
                              help="goals file")
    driver_other.add_argument("--plan-type",
                                help="type of plan to reach the state: shortest,metric-related")
    ...
    # ALBERTO POZANCO
    if args.state:
        args.search_options.append('--state')
        args.search_options.append(args.state)
    return args
