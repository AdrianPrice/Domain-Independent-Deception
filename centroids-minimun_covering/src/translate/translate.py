...
def translate_strips_conditions(conditions, dictionary, ranges,
    ...
    #ALBERTO POZANCO: Cambiado para que no compruebe que las metas son imposibles de alcanzar
...
def translate_task(strips_to_sas, ranges, translation_key,
    ...
    #goal_pairs = [(key,)+tuple(val) for dic in goal_dict_list for key,val in dic.items()]
    goal_pairs = []
    for x in range(len(goal_dict_list)):
        goal_pairs+=list(goal_dict_list[x].items())
    goal_pairs2 = list(goal_dict_list[0].items())
    if not goal_pairs:
...
def main():
    ...
    for predicate in task.init:
        if type(predicate) is pddl.f_expression.Assign:
            ...
            data = '(= (' + fluent + ' ' + ' '.join(args2) + ') ' + str(value) + ')'
            outfile.write(data + '\n')
            ...
            print("TREMENDO FALLO DEL TRANSLATOR QUE NO HA TENIDO EN CUENTA QUE HABIA UN NEGATED ATOM")
    outfile.close()
