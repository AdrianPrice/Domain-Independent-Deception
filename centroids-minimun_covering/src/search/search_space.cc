...
void SearchNode::open_initialB(int distance) {
    assert(info.status == SearchNodeInfo::NEW);
    info.status = SearchNodeInfo::OPEN;
    info.g = distance;
    info.real_g = distance;
    info.parent_state_id = StateID::no_state;
    info.creating_operator = OperatorID::no_operator;
}
...
void SearchSpace::trace_path(const GlobalState &goal_state,
    ...
    //cout << "Intento trazar el plan " << endl;
    GlobalState current_state = goal_state;
    /*vector <string> prube = current_state.return_pddl();
    for (auto t: prube){
        cout << t << endl;
    }*/
    assert(path.empty());
    //cout << "Esta vacio con lo que puedo hacer cosas " << endl;
    for (;;) {
        const SearchNodeInfo &info = search_node_infos[current_state];
        //cout << info.creating_operator.get_index() << endl;
        if (info.creating_operator == OperatorID::no_operator) {
