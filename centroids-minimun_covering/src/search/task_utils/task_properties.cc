...
void dump_pddl(const State &state) {
    ...
    for (FactProxy fact : state) {
        ...
        //cout << fact_name << endl;
        string atom = fact_name.substr(0,fact_name.find(delimiter));
        string predicate = fact_name.substr(fact_name.find(delimiter)+1,fact_name.size());
        string toQuit = "()";
        std::string::size_type i = predicate.find(toQuit);
        if (i != string::npos){
            predicate.erase(i,toQuit.length());
            predicate.insert(0,"(");
...
vector <string> return_pddl(const State &state) {
    vector <string> pddl;
    string delimiter = " ";
    for (FactProxy fact : state) {
        string fact_name = fact.get_name();
