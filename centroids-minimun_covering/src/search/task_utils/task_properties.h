...
// ALBERTO POZANCO
// We switch the condition to return true if any of the atomic goals is present in the current state
inline bool is_goal_state_pozanco(const TaskProxy &task_proxy, const GlobalState &state) {
    for (FactProxy goal : task_proxy.get_goals()) {
        FactPair goal_fact = goal.get_pair();
        if (state[goal_fact.var] == goal_fact.value) {
            return true;
...
extern std::vector<std::string> return_pddl(const State &state);
